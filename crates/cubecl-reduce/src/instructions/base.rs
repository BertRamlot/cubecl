...
pub trait ReduceFamily: Send + Sync + 'static + std::fmt::Debug {
    type Instruction<In: Numeric>: MonoidOperation<In, Config = Self::Config>;
...
#[cube]
pub trait MonoidOperation<In: Numeric>:
...
pub fn reduce_inplace<In: Numeric, R: MonoidOperation<In>>(
    ...
    let reduction = &R::operate(inst, accumulator, item, coordinate, use_planes);
...
pub fn reduce_shared_inplace<In: Numeric, R: MonoidOperation<In>>(
    ...
    let reduction = R::operate(inst, &acc_item, item, coordinate, use_planes);
