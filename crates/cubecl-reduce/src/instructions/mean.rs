...
use super::{ReduceCoordinate, ReduceFamily, MonoidOperation, ReduceRequirements, Sum};
...
impl<In: Numeric> MonoidOperation<In> for Mean {
    ...
    fn identity_input(this: &Self, #[comptime] line_size: u32) -> Line<In> {
        Sum::identity_input(&this.sum, line_size)
    }

    fn identity_accumulator(this: &Self, #[comptime] line_size: u32) -> Self::AccumulatorItem {
        Sum::identity_accumulator(&this.sum, line_size)
    }
    ...
    fn operate(
        ...
        Sum::operate(&this.sum, accumulator, item, _coordinate, use_planes)
    }
